...
```
1  폴백 소진 플래그를 본다.  서 있으면 큐에 넣지 않고 마감 응답
2  큐에 넣고 요청 예산만큼 잔다
```
...
|---|---|---|
| 발급 | 슬롯을 잡고 커밋까지 끝났다 | |
| **마감** | 확정 조회가 0행이었다 | 무의미하다 |
| **혼잡** | 요청 예산이 먼저 끝났다 | 가치 있다 |

**마감을 정하는 것은 확정 조회 하나뿐이고, 요청 예산은 언제 포기하는지만 정한다.** 둘을 섞으면 혼잡을 마감으로 답해 남은 재고를 못 팔거나, 마감을 혼잡으로 답해 끝난 이벤트에 재시도를 유도한다.
