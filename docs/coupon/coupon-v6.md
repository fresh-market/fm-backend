...
**검토했으나 채택하지 않은 안이다.** `docs/coupon/coupon.md` 7장이 접은 이유를 적고 있고, 이 문서는 그때 검토한 내용을 남겨 둔 것이다.

접은 이유를 요약하면 이렇다. **폴백을 두지 않기로 하면서 "Redis 가 죽어도 락이 없다" 가 무의미해졌고**, 남은 "구멍 회수가 싸다" 는 `pending` 키가 정상 경로를 맡으면서 **Redis 재건 때만 쓰이게** 됐다. 그 하나를 위해 DDL 을 다섯 군데 바꾸는 거래가 안 맞았다.

**10만 이상 규모에서는 다시 볼 값이 있다.** 아래 내용은 그때를 위해 남긴다.
...
지켜야 할 것이 둘이다. **스키마가 대신 지켜주지 않는 앱 코드 규율이다.**

```
1  배치 안에서 회원 중복을 제거한다
...
`free` 목록도 복구 시 다시 만든다. **아직 나가지 않은 뒷번호까지 담으면 안 된다.** `PENDING` 이라는 조건만으로 고르면 소진되지 않은 슬롯이 전부 딸려 들어온다.
...
`idx_pending (coupon_id, status, issue_seq)` 범위 조회라 싸다. v2~v5 는 구멍이 존재하지 않는 행이라 `1..MAX` 를 생성해 LEFT JOIN 해야 했다.

**이 인덱스가 v6 이 추가로 요구하는 유일한 것이다.** 발급 경로는 `uk_mc_coupon_seq` 정확 일치라 새 인덱스가 필요 없고, 구멍 회수만 이것을 쓴다.
...
`LOAD INDEX INTO CACHE` 는 MyISAM 전용이라 InnoDB 에는 안 먹는다. **이벤트 준비 절차가** 아래 조회를 돌려 올린다. 사전 삽입을 마친 뒤다.
...
**페일오버 뒤에는 같은 조회를 다시 돌린다.** 돌리는 주체는 이벤트 준비 절차와 같다. 9장의 RDS 페일오버는 버퍼 풀이 빈 인스턴스로 넘어가는 것이라 예열이 통째로 날아간다. 재개 직후 첫 배치들이 디스크를 읽게 되므로, 승격을 감지하면 예열을 다시 돌리는 절차가 필요하다.
