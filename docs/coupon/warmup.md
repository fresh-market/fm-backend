...
요청을 흘려 경로를 데우고, 워밍업이 끝난 뒤에야 트래픽을 받는다.
...
**정합성은 차가워도 지켜진다.** 초과 발급 0, 번호 유실 0, 실패율 0.00% 였다. 못 지키는 것은
p99 하나다.
...
**인스턴스는 죽지 않는다.** 전용 ASG 는 `health_check_type` 이 `EC2` 라(팀이 일부러 그렇게 뒀다)
readiness 실패로 교체하지 않는다. 인스턴스는 살아서 트래픽만 못 받으므로 **자동 복구가 없고
로그가 유일한 단서다.**

그래서 팀에게 `max-duration` 이 필요하다. 상한의 천장은 스크립트의 600초이지
`health_check_grace_period` 300초가 아니다. 그 300초는 EC2 상태 검사에 걸린 것이라 readiness 와
무관하다.
...
팀이 발급까지 가게 두면 `fk_mc_member` 가 워밍업용 회원 행을 요구하고 그 행이 운영 데이터에 남는다.
...
`CouponWriteWarmup` 은 워밍업 회원과 그 회원의 발급분을 **한 트랜잭션 안에서 넣고 통째로
롤백한다.** 외래 키 검사는 같은 트랜잭션 안을 보므로 통과하고, 워밍업이 되돌리면 회원 행까지
사라진다.
...
순환이라 팀이 발급까지 태우지 않고는 못 넘는다. 팀은 그것을 아직 안 했다.
...
로그로 넘어가 부하 시험 분석을 더럽힌다. 팀이 라운드를 실제 플러시 한 번 크기로 자르면 둘 다
자연히 지켜진다.

**식별자는 음수 대역이다.** `member_id` 가 AUTO_INCREMENT 라 실제 회원은 늘 양수다. 워밍업이 음수를
명시로 넣으면 양수 대역과 절대 안 겹치고 카운터도 앞으로 안 밀린다. 워밍업이 인스턴스마다 대역
안의 자리를 임의로 골라 세 대가 서로의 롤백을 안 기다린다.

**`issue_limit` 은 실제 수량이 아니라 라운드 크기를 쓴다.** `chk_mc_issue_seq` 가 순번이 1 이상
`issue_limit` 이하이기를 요구하는데 워밍업 쿠폰의 수량은 1 이라, 워밍업이 그대로 쓰면 둘째 행부터
...
**행당 비용의 대부분이 같은 코드라 팀이 데우려던 것은 데워진다.** 다만 워밍업은 운영이 안 쓰는
트랜잭션 관리 코드를 덤으로 데우고 커넥션 획득은 다른 경로를 탄다. 워밍업과 운영이 똑같지는 않다.

### 한 행도 안 남는 것을 지키는 테스트

**팀은 데우는 것을 로그로 보지만 "안 남는다" 는 눈으로 못 본다.** 롤백이 빠지거나 커밋으로
바뀌면 가짜 회원과 가짜 발급분이 쌓이는데 팀이 그것을 알아차릴 다른 장치가 없다.
`CouponWriteWarmupIntegrationTest` 가 그 자리를 맡는다.

## 5. 물렸던 것 셋

### 카운터를 안 세우면 안 데워지는 이유

**카운터가 없으면 순번 확보 스크립트가 첫 줄에서 `-2` 를 돌려주고 끝난다.** 요청은 데우려던
경로를 하나도 안 지난다. 게다가 그 `-2` 가 재건기를 깨운다. 재건기는 "켜져 있고 수량이 있는 쿠폰의
...
**팀은 타임아웃을 올려서 풀지 않는다.** 100ms 는 SLO 에서 역산한 값이고(왕복 2회 + 확정 대기 800ms
...
**팀은 카운터에 TTL 을 건다(1시간).** ElastiCache 기본 축출 정책이 `volatile-lru` 라 만료가 없는 키는
축출 대상에서 아예 빠진다. 팀이 워밍업 카운터를 영구로 두면 메모리가 찼을 때 실제 발급 상태인 `seq`
와 `pending` 이 먼저 날아간다. 팀이 워밍업 쿠폰 ID 를 바꿔 배포할 때마다 옛 키가 하나씩 쌓이는
문제도 같이 걸린다.
...
| 키 | 값 | 근거 |
|---|---|---|
| `enabled` | `true` | 팀이 로컬과 시험에서는 끈다 |
| `coupon-id` | `1000000` | V33 이 심는 전용 쿠폰이고 999999 는 이 저장소가 "없는 것" 으로 쓰는 번호라 팀이 피했다 |
| `requests` | `3000` | 팀에게 200 으로는 모자랐다 (최대 응답 1.72초) |
| `concurrency` | `20` | 러너가 순차로 보내면 실제 이벤트의 동시성을 못 흉내 낸다 |
| `max-duration` | `60s` | 3,000건이 약 30초이고 천장은 스크립트의 600초다 |
| `write-rows` | `5000` | 실제 이벤트가 만 행이라 그 절반이고 팀이 논증으로 잡은 시작값이다 |
| `write-timeout` | `20s` | 한 라운드의 트랜잭션 상한이고 정상이면 수백 밀리초에 끝난다 |

**팀은 값을 AWS 회차에서 정한다.** 팀이 로컬에서 정하지 않는 이유는 그쪽 MySQL 이 매 회차 새 컨테이너라
개선의 일부가 JIT 이 아니라 버퍼 풀이 데워진 몫이고, k6 와 JVM 과 DB 가 같은 CPU 를 다투기
때문이다. 팀이 그렇게 고른 값을 운영에 박으면 근거 없는 값이 근거 있어 보인다.
...
**팀은 부하 시험 지표를 읽을 때 주의한다.** 워밍업이 보낸 3,000건이 `coupon_issue_results_total` 의
`sold-out-final` 에 그대로 더해진다. `sold-out` 이 아니다. 팀은 회차별 수치를 절대값이 아니라 회차
시작 전후의 차이로 읽는다.
