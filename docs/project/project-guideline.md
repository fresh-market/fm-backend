...
버전은 [tech-stack.md](./tech-stack.md) 에 있고 **Spring Boot BOM 이 관리하는 것은 팀이 버전을 안 박는다.**

## 작업 흐름

작업은 이슈에서 시작한다. 팀원은 [프로젝트 보드](https://github.com/orgs/fresh-market/projects/6) 에서 무엇을 잡을지 고른다.
...
**팀원은 이슈 없이 시작하지 않는다.** 보드에 없는 작업은 남들이 모르고, 같은 것을 두 사람이 잡는다.
...
팀원은 PR 을 열면 카드를 **`In Review`** 로 옮긴다. 누가 리뷰를 기다리는지 보이게 하는 자리다.

`/v-commit` 은 **Claude Code 를 띄운 뒤 그 안에서 치는 슬래시 명령**이다. 터미널에서는 동작하지 않는다.
팀원이 다른 CLI 를 쓰면 터미널에서 `./verify.sh --agent <명령>` 으로 넘긴다.
...
| | |
|---|---|
| `*.internal.service.*` 메서드 커버리지 **100%** | 여유가 없어서 팀원이 서비스 메서드를 만들면 테스트도 만든다 |
| SonarQube 신규 **Blocker 0건** | |
| 다른 팀원 승인 **1건** | 자기 PR 은 자기가 승인 못 한다 |

**팀원은 `main` 에 직접 못 올린다.** 팀원은 `develop` 으로만 PR 을 연다.

LLM 판정(G-PR)은 지적만 하고 막지 않는다. 팀원이 읽고 판단은 한다.
...
[`domain-map.md`](../code-architecture/domain-map.md) 는 13개 도메인과 층을 정해 둔 것이다. **도메인은 호출을 아래로만 하고 같은 층끼리는 안 부른다.**
`ArchitectureTest` 가 빌드에서 강제하므로 팀원이 어기면 `./gradlew check` 가 실패한다.

## 문서 작성

| 어디에 | 무엇을 |
|---|---|
| `docs/wiki/` | 팀 문서와 문제 해결 공유이고 GitHub 위키로 자동 반영된다 |
| `docs/code-architecture/` | 코드와 함께 봐야 하는 기준이고 팀원이 직접 쓰지 않는다 |
| `docs/llm-review/` | 검증 기록이고 팀원이 직접 쓰지 않는다 |
