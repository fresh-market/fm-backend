...
**판매 마감이 지난 로트는 대상이 아니다.** 팔 수 없는 재고에 쿠폰을 붙여도 회원이 쓸 수가 없다.
...
**응답에 객체 키를 따로 담지 않는다.** 키는 `uploadUrl` 경로 안에만 있다. 클라이언트가 키를 돌려받아 저장하면
...
서버는 presigned URL 을 매번 새로 서명해서 돌려준다. URL 자체엔 만료 시간이 있어, 재시도 응답도 그 자리에서 바로 쓸 수 있어야 한다.

대표 이미지는 상품당 하나다. 교체할 때는 옛 대표를 먼저 내려야 한다.

**삭제는 소프트 딜리트가 아니라 물리 삭제다** (`API-5-09`). 다른 어떤 테이블도 이미지 행을 참조하지
않아 되살려야 할 이유가 없고, 실수로 지웠다면 재업로드가 곧 복구다. undelete 와 만료와 show_deleted 는
두지 않는다. **이미 지워진(또는 존재한 적 없는) `imageId`를 다시 삭제해도 오류가 아니다.** 목표
상태(행 없음)에 이미 도달한 것으로 본다 (`API-3-07`, `AIP-135`).
