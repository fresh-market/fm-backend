...
쿠키 등) 도입 여부는 팀이 아직 정하지 못했다([정하지 못한 것](#정하지-못한-것) 참고). 회원 인증 체인은 현재 `SameSite=Strict` 를 사용하며 CSRF 토큰 방식은 아직 정하지 않았다.
...
| `204` | | 본문 없음 |
| `401` | | 로그인 상태가 아니다 |
| `500` | `AUTH-006` | DB 백업/Redis 정리 중 하나라도 실패. **재시도 가능하다** |
...
DB의 Refresh Token 폐기, Redis의 Refresh Token 정리, Access Token 차단은 서버에서 각각 한 번만 시도한다.
DB 또는 Redis 작업이 실패하면 서버 내부에서 Jitter 재시도나 스케줄러 재처리를 수행하지 않고 실패 로그를 남긴 뒤 `ADMIN-010`으로 응답한다.
이후 재시도는 클라이언트가 새 로그아웃 요청을 보내는 방식으로 수행한다.

Access Token 차단은 기존 공용 `JwtAuthenticationFilter`의 Redis 장애 시 fail-open 정책을 변경하지 않는다.
로그아웃 요청에서 커트라인 저장이 실패하면 성공으로 응답하지 않고 `ADMIN-010`을 반환한다.

로그아웃 성공 시 `accessToken`, `refreshToken` 쿠키를 모두 `Max-Age=0`으로 만료한다.
관리자 Refresh Token 쿠키는 발급 시와 동일한 `Path=/v1/admin/auth/`를 사용한다.

| 응답 | 코드 | 언제 |
|---|---|---|
| `204` | | 로그아웃 성공 |
| `404` | `ADMIN-009` | 인증 주체에 해당하는 관리자 계정을 찾을 수 없음 |
| `503` | `ADMIN-010` | DB 또는 Redis의 필수 로그아웃 처리를 완료하지 못함 |

`ADMIN-010`은 DB 또는 Redis의 일시적인 장애로 필수 로그아웃 처리를 완료하지 못한 경우의 **재시도 가능한 오류**다.
서버는 같은 요청 안에서 다시 시도하지 않는다. 클라이언트는 `503 ADMIN-010`을 받으면 즉시 연속 호출하지 않고 1초부터 시작하는 지수 백오프(1초 -> 2초 -> 4초)로 최대 3회까지 새 로그아웃 요청을 보낼 수 있다.
...
| `refresh_token_hash` | 인증 자격증명 파생정보(고민감) | `admin`에 SHA-256 해시만 저장하고 DB 접근 권한으로 제한 | 해시 값 자체를 애플리케이션 로그에 남기지 않음 | 활성 세션에서는 RT 만료/폐기 시 제거 |
| `audit_log` | 관리자 보안 감사 이력 | 업무 DB 접근 통제와 변경 이력 보호 대상 | 비밀번호와 토큰 원문 등 자격증명 기록 금지 | 프로젝트의 관리자 감사 이력 보존 정책이 확정되면 그 기간을 따른다. 현재 별도 보존 기간은 정하지 않았다 |

#### 로그아웃 실패 처리

로그아웃 실패를 별도 DB 실패 테이블에 저장하거나 스케줄러로 재처리하지 않는다.
DB/Redis 필수 작업 실패는 애플리케이션 로그에 필요한 최소 정보만 남기고 `ADMIN-010`으로 응답한다. Refresh Token 원문이나 해시는 실패 로그에 기록하지 않는다.
클라이언트가 새 로그아웃 요청을 보내면 DB RT 폐기와 Redis 조건부 삭제의 멱등성을 이용해 처음부터 다시 처리한다.

RT 폐기와 Access Token 차단까지 완료된 뒤 감사 로그 저장만 실패한 경우에는 보안상 로그아웃은 이미 완료된 상태이므로 `ADMIN-010`으로 되돌리지 않는다. 감사 로그 저장 실패는 애플리케이션 오류 로그만 남기고 로그아웃 성공 응답을 유지한다.
