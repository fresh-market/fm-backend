...
상품 이미지 업로드도 등록과 분리된 별도 흐름이다. 해당 내용은 [상품 이미지 업로드.md](./상품%20이미지%20등록.md) 문서를 따로 둔다.
...
다만 두 요청이 동시에 들어와 뒤의 요청이 저장 시점에 락 대기까지 갔는데 그 사이 앞 요청이 아직
커밋을 못 마친 경우에는, PessimisticLockingFailureException으로 대기가 끝나며 그 즉시 재조회를 한 번 더 한다.
그 사이 앞 요청이 커밋을 마쳤으면 그 결과를 그대로 돌려주고, 그마저 없으면 아직 처리 중이라는 의미로
REGISTRATION_IN_PROGRESS(409)를 응답한다. 클라이언트는 이 경우 잠시 후 같은 requestId로 다시 요청하면 된다.
...
sequenceDiagram
    ...
    S->>DB: product 저장 (상품코드는 저장 전에 이미 완성됨, 4장)
    alt supplierId가 실재하지 않음 (FK 위반)
        S-->>A: 404 (SUPPLIER_NOT_FOUND)
    end
    alt 동시 재시도가 아직 커밋 전 (락 대기 타임아웃)
        S-->>A: 409 (REGISTRATION_IN_PROGRESS, 재조회해도 커밋된 결과가 없을 때)
    end
    S->>DB: product_option 저장 (옵션별로 개별 저장)
    alt 같은 상품 안에 옵션명 중복
        S-->>A: 409 (OPTION_DUPLICATE_NAME)
    end
    DB-->>S: 커밋 완료
...
그 외에 두 가지 오류 케이스가 더 있다. 하나는 같은 상품 안에서 옵션명이 겹치는 경우다. 
product_option에도 UNIQUE 제약이 있어, 옵션을 하나씩 저장하는 도중 위반이 나면
OPTION_DUPLICATE_NAME(409)으로 바꿔 응답한다. 다른 하나는 같은 requestId를 가진 두 요청이 동시에 들어와,
뒤의 요청이 product 저장 시점에 락 대기를 하다 타임아웃난 경우다. 이때는 앞 요청이 그 사이 커밋을 마쳤으면
재조회해 그 결과를 그대로 돌려주고, 아직 커밋 전이면 REGISTRATION_IN_PROGRESS(409)로 응답해 클라이언트가
잠시 후 재시도하도록 안내한다.
...
| 방안 | 개요 | 장점 | 단점 |
|---|---|---|---|
| A. 임시값 INSERT 후 `product_id`로 실제 값 UPDATE | `NOT NULL`을 임시값으로 우회하고, INSERT 직후 확정된 `product_id`로 같은 트랜잭션 안에서 덮어쓴다 | 별도 테이블과 락과 재시도가 불필요 | 코드에 내부 PK 정보가 드러남. INSERT 두 번(또는 INSERT+UPDATE) 필요 |
...
| D. 영숫자 무작위 6자리 (채택) | `P-{연도}-{영숫자 6자리 무작위}`. `product_id`나 다른 재료에 전혀 의존하지 않고 INSERT 전에 완성 | 순서 문제 자체가 없음. 별도 테이블과 락과 재시도와 UPDATE 가 전부 불필요. 내부 PK도 드러나지 않음 | 이론상 충돌 가능성이 0은 아님(4.5절) |

D는 A~C가 공통으로 전제한 "코드는 `product_id`에서 파생되어야 한다"는 가정 자체를 버린 방안이다.
36자(0-9, A-Z) 중 6자리 조합은 경우의 수가 실제 등록 빈도에서 충돌 확률이 무시할 수준으로 낮다. 
이 낮은 충돌 확률과, A~C가 요구하는 별도 인프라와 순서 제어가
...
위 표는 필드 단위 제약이다. 이 외에 `options[].name`은 같은 상품 안에서 유일해야 한다는
비즈니스 규칙이 있다(`uk_option_product_name`). 이 규칙은 요청 형식 자체의 문제가 아니므로 위반해도
400이 아니라 409(`OPTION_DUPLICATE_NAME`)로 응답한다.

## 목록과 단건 조회

등록과 별도로, 관리자용 목록과 단건 조회도 같은 서비스(`AdminProductService`)에 함께 구현되어 있다.
...
- 판매안함과 품절과 삭제 상품까지 전부 조회 대상이다. `includeDeleted`로 삭제 상품 포함 여부만 가른다.
