...
/**
 * 선착순 발급의 조율 값이다. {@code docs/coupon/coupon-v4.md} 3장이 "재서 정할 값" 으로 잡은 것들이라
 * 기본값은 시작점일 뿐이고 부하 시험에서 옮긴다.
 *
 * <p>흩어 두지 않고 한곳에 모은 이유가 둘이다. 하나는 이 값들이 서로의 상하한이라 따로 고치면
 * 계층이 역전된다는 것이고, 다른 하나는 시험에서 함께 움직이는 값이라 한 파일에 있어야
 * 무엇을 바꿔 무엇이 달라졌는지 읽힌다는 것이다.
 *
...
public record CouponIssueProperties(
    ...
    public CouponIssueProperties {
         ...
         * 그 둘이 같은 번호로 INSERT 해 uk_mc_coupon_seq 에 걸리므로 기동에서 막는다.
         */
