...
public interface MemberRepository extends JpaRepository<Member, Long> {
    ...
    @Query("update Member m set m.status = com.freshmarket.member.internal.entity.MemberStatus.WITHDRAWN, "
            + "m.deletedAt = :deletedAt where m.id = :id and m.status not in "
            + "(com.freshmarket.member.internal.entity.MemberStatus.WITHDRAWN, com.freshmarket.member.internal.entity.MemberStatus.WITHDRAWN_FAILED)")
    int markWithdrawn(@Param("id") Long id, @Param("deletedAt") LocalDateTime deletedAt);

    @Modifying(clearAutomatically = true)
    @Query("update Member m set m.status = com.freshmarket.member.internal.entity.MemberStatus.WITHDRAWN_FAILED "
            + "where m.id = :id and m.status = com.freshmarket.member.internal.entity.MemberStatus.WITHDRAWN")
    int markUnlinkFailed(@Param("id") Long id);

    @Modifying(clearAutomatically = true)
    @Query("update Member m set m.status = com.freshmarket.member.internal.entity.MemberStatus.WITHDRAWN "
            + "where m.id = :id and m.status = com.freshmarket.member.internal.entity.MemberStatus.WITHDRAWN_FAILED")
