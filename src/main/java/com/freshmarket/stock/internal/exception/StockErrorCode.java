...
public enum StockErrorCode implements ErrorCode {
     ...
     *
     * 자정 스케줄과 관리자 재실행이 같은 확정 로직을 쓰므로 겹칠 수 있다. 뒤엣것은
     * campaign_rebuild_lock 을 못 잡고 기다리지도 않고(NOWAIT) 떨어지는데, 그 상황을 500 이
     * 아니라 "지금은 안 되니 잠시 후" 로 알린다.
     *
     * 잠금이 DB 행 잠금이라 인스턴스를 넘어 겹쳐도 이 코드로 온다. 예전처럼 애플리케이션
     * 플래그로 걸렀다면 같은 JVM 안에서 겹친 것만 여기로 오고, 배치 인스턴스와 API 인스턴스가
     * 겹치는 경우는 데드락이 되어 500 으로 샜을 것이다.
     */
