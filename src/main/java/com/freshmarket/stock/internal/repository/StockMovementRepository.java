...
import com.freshmarket.stock.internal.entity.StockMovement;
...
public interface StockMovementRepository extends JpaRepository<StockMovement, Long> {
     ...
     *
     * RETURNED 는 제외한다. 회수품이 판매 재고로 돌아온 적이 없어 available_qty 를 줄이지
     * 않는 폐기라(AdminLotService.dispose 와 chk_movement_qty 가 그렇게 강제한다),
     * 분모(입고 - 폐기)에서 빼면 available_qty 가 분모보다 커져 소진율이 음수가 된다.
     */
    @Query("select new com.freshmarket.stock.internal.dto.LotDisposedQty(m.stockLotId, sum(m.quantity)) "
            + "from StockMovement m "
            + "where m.movementType = com.freshmarket.stock.internal.entity.MovementType.DISPOSE "
            + "and m.disposalReason <> com.freshmarket.stock.internal.entity.DisposalReason.RETURNED "
