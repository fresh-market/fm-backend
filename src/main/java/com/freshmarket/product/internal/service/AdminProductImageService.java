...
import com.freshmarket.product.internal.client.S3ObjectMetadata;
...
public class AdminProductImageService {
     ...
     *
     * (API-5-09) 소프트 딜리트로 가지 않는다. 어떤 테이블도 product_image 행을 참조하지 않는
     * leaf 엔티티라 되살려야 할 이유가 없고, 실수로 지웠다면 재업로드가 곧 복구다. undelete를
     * 만들려면 S3 객체까지 같이 살려둬야 의미가 있는데(그래야 진짜로 되돌릴 게 남는다), 그러면
     * 정리 배치·보관 기간 정책이 통째로 새로 필요해진다 — 상품(참조 무결성 때문에 소프트 딜리트가
     * 필수)과 달리 그 비용을 들일 이유가 없다.
     *
