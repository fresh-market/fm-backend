 ...
 *
 * requestId는 재시도 감지에 쓴다(API-5-07, AIP-155) — register()/dispose()와 같은 이유다.
 */
public record AdminProductImageCreateUploadUrlRequest(
        ...
        @Schema(description = "업로드할 파일의 MIME 타입. REQUIRED", example = "image/jpeg")
        @NotBlank @Size(max = 100) String contentType,
        @Schema(description = "업로드할 파일의 바이트 크기. REQUIRED", example = "482913")
