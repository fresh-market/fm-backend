...
class ExpiringSoonServiceTest {
    ...
    // 대상 로트 하나. id 는 엔티티가 채우는 값이라 테스트에서 직접 넣는다
    ...
    void 대상_로트의_상품_정보를_붙여_돌려준다() {
        ...
        when(productApi.findOptionInfos(List.of(31L))).thenReturn(List.of(info(31L, 4L, true)));
    ...
    void 구매할_수_없는_상품은_빠진다() {
        when(campaignTargetLotRepository.findByTargetDateAndTargetRankGreaterThanOrderByTargetRankAsc(
                eq(TODAY), eq(0), any())).thenReturn(List.of(targetLot(100L, 1)));
        when(stockLotRepository.findAllById(List.of(100L))).thenReturn(List.of(stockLot(100L, 31L)));
        when(productApi.findOptionInfos(List.of(31L))).thenReturn(List.of(info(31L, 4L, false)));
    ...
    void 결과가_pageSize보다_많으면_다음_페이지_토큰을_준다() {
        // given — pageSize 1, 여유분(FETCH_MULTIPLIER=2)까지 2건을 채우고도 남아 다음 페이지가 있다
        when(campaignTargetLotRepository.findByTargetDateAndTargetRankGreaterThanOrderByTargetRankAsc(
                eq(TODAY), eq(0), any()))
        ...
        when(stockLotRepository.findAllById(List.of(100L, 200L)))
                .thenReturn(List.of(stockLot(100L, 31L), stockLot(200L, 32L)));
        when(productApi.findOptionInfos(List.of(31L, 32L)))
                .thenReturn(List.of(info(31L, 4L, true), info(32L, 4L, true)));
