...
```
                     클라이언트
                         |
        Route 53 --- ALB (2 AZ, HTTPS 443)  --- CloudFront + OAC --- S3 (비공개)
                     /        \
      [퍼블릭]  app ASG      coupon ASG          batch          monitoring
                t3.small     t3.small           t3.small       t4g.small
                x 1~3        x 0~3 (이벤트)     스케줄러 전용   Prometheus/Grafana/Loki
                     \        /
      [프라이빗]  RDS MySQL 8.4 (Multi-AZ)   ElastiCache Valkey 9.0 (primary + replica)
```

| | |
|---|---|
| 월 비용 | 약 140~181 USD (앱 1~3대) |
| 앱이 퍼블릭 서브넷에 있는 이유 | NAT Gateway 가 월 40 USD, **예산의 1/4**. 보안은 서브넷이 아니라 보안 그룹으로 확보한다 |
| 열려 있지 않은 것 | **22번 포트가 없다.** 운영 접근은 SSM Session Manager, Grafana 도 SSM 포워딩으로 본다 |
| 포트 개방 방식 | 전부 보안 그룹 참조. CIDR 을 직접 쓰지 않는다 |
| 관측 | 관리형(월 91~95 USD) 대신 자체 호스팅(월 14 USD). **관리형은 스크랩 간격을 좁힐수록 비싸지는데**, 30초짜리 장애를 보려면 5~15초 해상도가 필요하다 |
| 배포 | `main` 병합 -> GitHub Actions -> OIDC 로 AWS STS -> ASG desired +1 -> 헬스체크 통과 후 구 인스턴스 종료. 6~8분, 롤백 1~2분 |
...
**나누는 이유는 격벽이다.** 안 나누면 선착순 트래픽이 톰캣 스레드와 커넥션 풀을 다 먹어 상품 조회와 주문이 함께 죽는다. 부수 효과로 버전 비교가 깨끗해지고 되돌리기가 쉬워진다. `coupon_dedicated_enabled` 를 끄면 규칙만 사라지고 발급 요청은 평상시 앱으로 흐른다.
...
19:38~19:45 KST 회차의 Grafana 캡처를 읽은 것이다. 캡처에 날짜가 없어 회차 번호는 기록 문서와 대조해야 하지만, 발급 10,000 / 소진 10,000 으로 요구 부하와 같은 시나리오다.

| 패널 | 본 것 | 무엇을 뒷받침하나 |
|---|---|---|
| 응답 종류별 누적 | 발급 200 과 소진 409 가 **각각 정확히 10,000 에서 평평해진다.** 200 이 먼저 닿고 409 가 뒤따른다 | 재고를 다 팔았고 그 뒤 요청은 전부 소진으로 갈렸다 |
| 봐서는 안 되는 것 | **No data** | 초과 발급과 서버 오류 계열이 한 건도 없다 |
| 회로 상태 / 즉시 거절 | 값 없음 / **0 req/s** | 회로가 한 번도 열리지 않았고 거절도 0건이다 |
| **느린 호출 비율** | 램프 구간에 **60% 까지 두 번 튀었다가** 이후 0% 로 수렴. 임계선은 50% | **오진이 실재한다는 증거다.** metrics-only 로 안 바꿨으면 그 순간 회로가 열렸다 |
| 톰캣 스레드 | **No data** | 가상 스레드를 켜서 톰캣 스레드 풀 자체가 안 쓰인다 (v4) |
| 스레드 수 | 요청 2만 건 동안 **39개에서 평평** | 요청이 늘어도 플랫폼 스레드가 안 는다 |
| HikariCP 커넥션 | active 가 **2 에서 고정** | `maximum-pool-size: 2`. **요청이 2만이어도 커넥션 수요가 안 느는 것이 이 설계의 요점이다** |
| 프로세스 CPU | 램프 구간에 최대 70%, 그 뒤 0% 로 떨어진다 | `t3.small` 2 vCPU 에 여유가 남았다. 차가운 회차의 100% 포화와 대비된다 |
| 힙 / GC 일시정지 | 사용량 1 GiB 미만, 정지 **최대 1ms** | 회로 지표를 켜다 GC 정지를 330ms 로 만들었던 문제가 재발하지 않았다 (6.5) |
| 실패율 | **0%** | |

**꼬리가 서버 밖에 있다.** k6 의 응답 분해가 그것을 보여 준다.
...
**서버가 답하는 시간보다 생성기가 연결을 기다린 시간이 길다.** 소진이 나는 순간에 2만 VU 가 한꺼번에 연결을 잡으려 하기 때문이고, 6.1 에서 409 의 최대가 200 보다 두 배였던 것과 같은 현상이다. `connecting` 과 `sending` 이 0 에 가까우므로 네트워크나 전송이 아니라 **생성기 쪽 연결 확보**가 그 구간의 정체다.

배치 패널의 "마지막 성공 이후 경과" 는 `OptionAvailabilitySyncScheduler.retryPendingSyncs()` 가 10분까지 올라갔다 0 으로 떨어지는 톱니를 그린다. **톱니가 곧 살아 있다는 신호다** — 계속 올라가기만 하면 그 배치가 멈춘 것이다.
