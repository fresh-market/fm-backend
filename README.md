...
| [6. 장애](#6-장애) | 대체 경로를 새로 만들지 않는다. 그리고 회로가 오진했던 일 |
| [7. 부하 시험: 결과, 해석, 개선](#7-부하-시험-결과-해석-개선) | 워밍업이 p99 를 가른다 |
| [8. 도메인별 핵심 로직과 트러블슈팅](#8-도메인별-핵심-로직과-트러블슈팅) | 각자 맡은 영역 |
...
**1. 격벽이다.** 안 나누면 선착순 트래픽이 톰캣 스레드와 커넥션 풀을 다 먹어 **상품 조회와 주문이 함께 죽는다.** 이벤트가 90초 동안 CPU 90% 를 치는데, 그 부하를 평상시 경로와 같은 인스턴스에 태울 이유가 없다.

**2. 오토스케일링이 이 트래픽에는 안 듣는다**(`INF-40`). 확장 판정에 최소 3분이 걸리는데 **선착순 쇄도는 그보다 짧게 끝난다.** 알람이 울리고 인스턴스가 부팅되는 사이에 재고가 이미 소진된다. 그래서 전용 ASG 에는 정책을 걸지 않고 `min 0 / desired 0`, 대수는 `coupon-event.sh open 3` 이 **이벤트 전에 미리** 정한다. 끝나면 0 으로 내린다.

**3. 설정이 정반대다.** 평상시 앱은 풀을 넉넉히 쓰고 헬스체크를 촘촘히 보지만, 전용 인스턴스는 **풀을 2로 조이고 헬스체크를 느슨하게** 해야 한다. 한 그룹에 두면 어느 한쪽 기준으로 맞출 수밖에 없다.

**4. 되돌리기와 비교가 쉬워진다.** `coupon_dedicated_enabled` 를 끄면 ALB 규칙만 사라지고 발급 요청은 평상시 앱으로 흐른다. 경로가 막히지 않으므로 언제든 되돌릴 수 있고, 버전(v1~v4) 비교도 다른 트래픽에 오염되지 않는다.


**그리고 이것이 가장 크다 - 이 그룹은 쿠폰 부하만 받는다.** 평상시 앱과 인스턴스가 겹치지 않으므로 **다른 기능의 성능에 영향이 없고**, 잰 수치가 오직 발급 경로의 것이 된다. 그래서 운영에 올리기 전에 개발 서버에서 **같은 모양의 예측 가능한 환경**으로 미리 대비할 수 있다. 대수와 프로필과 경로가 같으면 개발에서 본 것이 운영에서도 그대로 나온다 - 7장의 회차별 비교가 성립하는 것도 이 조건 덕분이다.
...
**이벤트 구간에는 캐시 등급이 올라간다.** 평상시 Valkey 는 조회 캐시라 `Degradable`(죽으면 원본 조회로 강등)이지만, 이벤트 중에는 **순번의 판정 주체**라 `Critical` 로 승격해 2노드 + 자동 페일오버로 둔다. 키를 잃으면 DB 와 앱 큐에서 재건한다(6장).
...
**서버는 클라이언트의 신고를 그대로 믿지 않는다.** `CONFIRMED` 로 가는 경로가 서버의 직접 확인 하나뿐이라 깨진 링크가 원천 차단되고, 서비스에는 `CONFIRMED` 만 노출한다. 삭제는 반대 순서로 S3 를 먼저 지운다.
...
**팀은 뒤엣것을 골랐다.** `issue_limit` 은 행을 만들 때 `coupon.total_quantity` 를 복사한 값이라, **삽입 한 건이 다른 행을 안 보고 자기 행만으로 상한을 판정한다.**

**그래서 순번 발급기가 어디에 있든 초과 발급은 나지 않는다.** 카운터가 실제보다 작으면 `uk_mc_coupon_seq` 가 막고, 크면 `chk_mc_issue_seq` 가 막는다.

**이것이 이 설계의 바닥이다.** 안전을 스키마가 책임지므로 **버전 사이의 차이가 "안전한가" 가 아니라 "얼마나 빠르고 얼마나 자주 실패하는가" 가 된다.** 위쪽 구현을 v1 에서 v7 까지 자유롭게 바꿔 볼 수 있었던 이유다.
...
**이벤트가 시작되면 조건이 얼어붙는다.** 앱은 시작된 이벤트의 일정을 못 바꾸고, 소진 전이고 마감 전이면 스위치를 못 끈다. 다섯 값이 전부 고정되므로 자격 확인을 캐시할 수 있고, 요청마다 하던 쿠폰 조회가 사라진다.
...
### 5.4 응답을 다섯으로 나눈 것이 설계의 절반이다

| 응답 | 상태 | 왜 이렇게 나누나 |
|---|---|---|
| 발급 | 200 | `{ "issueSeq": 6, "alreadyIssued": false }` |
| **이미 발급** | **200** | 서버는 "사용자가 다시 눌렀다" 와 "클라이언트가 자동 재시도했다" 를 구분 못 한다. 실패로 답하면 사용자가 못 받은 줄 알고 계속 누르고 그 요청이 다시 부하가 된다 |
| 소진, 회수 여지 | 409 | 미확정 번호가 남아 있다. 기준 시간이 지나면 그 번호가 다시 나온다 |
| **소진, 최종** | **410** | 다시 나올 번호가 없다 |
| 혼잡 | 503 + `Retry-After` | 앱이 순번을 못 받았거나 예산 안에 못 끝냈다. **재고는 남아 있다** |
...
**앱이 소진과 혼잡을 뭉치면** 재고가 남았는데 마감으로 답하거나, 끝난 이벤트에 재시도를 유도한다.
...
| v6 / v7 | 미리 넣은 행을 update 로 잡는다 | 결번이 행으로 실재한다 | **DDL 을 늘린다. 접었다** |
...
### 5.6 타임아웃 계층은 SLO 에서 역산했다

**팀이 스스로 건 p99 1초에서 안쪽 값들을 거꾸로 깎아 내려갔다.** 요청 예산 안에 배치 윈도와 커넥션 획득과 실행과 커밋과 확정 표시가 전부 들어가야 한다.

**계층이 역전되면 안 된다.** 안쪽(DB)이 바깥쪽(요청 예산)보다 길면 **요청 스레드가 먼저 포기해 혼잡으로 답한 뒤에 그 배치가 커밋된다.** 실패했다고 답했는데 발급된 상태가 되고, 사용자가 안 돌아오면 그 슬롯은 죽은 재고다.
...
**이 장은 결론만 적는다.** 앱 코드가 무엇을 하는지는 [failure-response.md](./docs/incident/failure-response.md), 발급 경로만의 이야기는 [coupon-failure.md](./docs/coupon/coupon-failure.md), 인프라가 무엇을 승격하고 얼마나 걸리는지는 [`fm-infra`](https://github.com/fresh-market/fm-infra) 가 갖는다.

### 6.1 원칙 - 대체 경로를 새로 만들지 않는다

**대체 경로는 평상시에 안 돌아 정작 필요한 날 처음 실행된다.** 그날 그 코드가 맞는지 아무도 모른다. 그래서 팀은 세 가지를 안 만들었다.

```
대체 순번 발급기   Redis 가 죽으면 앱이 새 요청을 혼잡으로 끊는다.  다른 곳에서 번호를 만들지 않는다
배치 페일오버      앱이 이어받게 만들지 않는다.  대신 작업 자체를 멱등하게 둔다
재건용 차단 장치   요청을 막는 장치를 새로 만들지 않는다.  스크립트가 카운터 없이 이미 막는다
```

**대신 팀은 이미 도는 경로가 장애도 받게 만들었다.** 재시도와 조건부 UPDATE 와 유니크 제약처럼 평상시에 늘 실행되는 것들이다.

**그리고 팀은 readiness 에 공유 의존성을 안 넣는다.** DB 를 넣으면 페일오버가 도는 동안 **모든 인스턴스가 동시에 대상 그룹에서 빠진다.** 한 대의 문제가 아니라 전체가 사라지는 것이라 상황이 더 나빠진다. **공유 의존성이 죽은 것은 인스턴스를 빼서 고칠 수 있는 문제가 아니다.**

### 6.2 무엇이 죽으면 앱이 무엇을 하나

| 죽은 것 | 인프라가 하는 일 | 앱이 하는 일 |
|---|---|---|
| ALB | 두 AZ 에 걸쳐 있어 남은 AZ 로 라우팅한다 | **없다** |
| 앱 인스턴스 | 컨테이너는 Docker 가 재시작하고, 인스턴스는 ASG 가 다른 AZ 에 다시 띄운다 | **없다.** 상태를 안 들어서 교체가 곧 복구다 |
| MySQL | Multi-AZ standby 를 자동 승격한다. 동기 복제라 커밋 손실이 없다 | 커넥션을 다시 얻고 재시도한다. 일시적 실패만 재시도로 가른다 |
| Valkey (조회 캐시) | 복제본을 승격한다 | **원본 조회로 강등한다.** 적재 실패는 무시한다 |
| Valkey (발급 순번) | 복제본을 승격한다 | 승격만으로 안 끝난다. **키가 비어 있으면 앱이 DB 와 큐로 다시 세운다** |
| 배치 인스턴스 | 대체를 두지 않는다 | **없다.** 다음 주기가 따라잡는다 |
| 모니터링 인스턴스 | 대체를 두지 않는다 | **없다.** 핵심 알람만 CloudWatch 로 두 번째 경로를 만든다 |

**일곱 중 넷은 앱이 할 일이 아예 없다.** 그 넷이 죽어도 발급과 주문이 안 멈춘다는 뜻이다.

**팀이 배치를 이중화하지 않은 것은 비용 때문이 아니다.** 프로세스가 항상 하나인 것이 전제라 겹쳐 돌면 같은 배치가 두 번 돈다. 그래서 팀은 이중화 대신 **상태 전이를 조건부 UPDATE 로 두어 겹쳐 돌아도 한 번만 반영되게** 만들었다.

### 6.3 회로가 재고를 지키지 않고 이벤트를 망가뜨렸다

**부하 시험에서 찾은 것 중 가장 큰 소득이다.** 팀이 같은 코드와 같은 차가운 JVM 에서 순번 확보 회로만 껐다.
...
무엇이 잘못이었나. **회로의 느림 판정 기준은 Redis 가 느린지 보라고 정한 값인데, 회로가 실제로 잰 것은 호출을 감싼 구간 전체였다.** 거기에는 가상 스레드가 CPU 를 얻기까지 기다린 시간이 들어간다.

```
차가운 JVM -> 요청당 CPU 5배 -> 2 vCPU 포화 -> 스케줄 대기가 측정에 섞임
  -> 느린 호출 100% -> 회로 열림 -> 재고가 남았는데 503
    -> 재시도로 요청 2.26배 -> CPU 100% -> 헬스체크 초과 -> ALB 퇴출 -> ASG 종료
```

**회로가 자기 앱의 느림을 Redis 장애로 오진했고, 그 오진이 진짜 장애를 만들었다.** 그동안 ElastiCache `EngineCPUUtilization` 은 3.7% 였다.

**고친 방법은 회로를 떼는 것이 아니다.** `CouponSeqAllocator` 가 이 회로를 지표 전용 상태로 넘긴다. **판정과 지표는 그대로 두고 거절만 안 한다.** 무엇이 느렸는지는 여전히 보인다.

**여기서 규칙 하나가 나왔다.** 느림 문턱은 그 호출의 타임아웃보다 뒤여야 한다. 앞에 두면 회로가 **대상의 상태가 아니라 자기 CPU 여유를 보게 된다.**

**팀이 이것을 밝히는 데 하루가 걸린 이유가 따로 있다.** 팀이 회로를 레지스트리 밖에서 만들어 **Micrometer 가 그것을 못 봤다.** 회로가 열려도 지표에 흔적이 없었다. 게다가 팀이 등록해서 고치려다 **지표를 보려던 행위가 지표가 재려던 현상을 만들었다.** 이벤트 소비자가 붙자 호출마다 객체가 생겨 GC 정지가 세 배가 됐다. 지금은 설정을 스타터로 옮겨 이 문제가 없다.

> 회로를 `couponSeq` 와 `couponWrite` 둘로 나눈 이유, 플러시를 안 막는 이유는 [coupon-failure.md](./docs/coupon/coupon-failure.md) 1장에 있다.

### 6.4 장애를 실제로 넣어 재 봤다

**팀은 AWS 에 인프라를 올려 가장 좋은 구성에 장애를 넣었다.** 앱 급사(SIGKILL), 캐시 Multi-AZ 페일오버, 둘을 겹친 것 셋이다. **팀은 DB 페일오버를 뺐다.** DB 가 죽으면 애초에 발급이 안 되므로 잴 것이 없다.

**실패는 장애 중이 아니라 회로가 닫히는 순간에 몰린다.** 캐시 페일오버 실측이 20초대인데 순번 확보가 막힌 구간은 45초를 넘었다.
...
**차이가 회로다.** 캐시가 돌아와도 회로는 반열림을 거쳐야 닫히고, **닫히는 순간 밀린 요청과 재시도가 한꺼번에 들어와 같은 회원의 요청이 겹친다.**

**여기서 회수 기준 시간의 근거가 바뀌었다.** 값은 60초 그대로다.
...
**둘의 차이가 수십 배다.** 약한 쪽만 적혀 있어서 팀이 10초로 내렸다가 망가뜨렸다. 회수 가드가 읽는 확정 표시는 `markCommitted` 가 붙이는데 **그것도 Redis 호출이라 Redis 가 죽으면 표시가 안 붙는다.** 회수가 **이미 DB 에 있는 번호를 남에게 넘겨** 유니크 제약에 걸린다.

> 60초와 30초와 10초를 재서 고른 과정은 [coupon-reclaim-after.md](./docs/coupon/coupon-reclaim-after.md), 회차별 값은 [coupon-load-rounds.md](./docs/coupon/coupon-load-rounds.md) 에 있다.
...
**그 뒤에 팀은 AWS 에 인프라를 올려 19회차를 더 돌렸다**(7.5). 같은 인프라에서 돌린 장애 회차는 6장이 갖는다. 회차를 왜 그렇게 짰는지는 [coupon-load-scenarios.md](./docs/coupon/coupon-load-scenarios.md), 회차별 값은 [coupon-load-rounds.md](./docs/coupon/coupon-load-rounds.md) 에 있다. 초기 회차 기록은 [`fm-infra`](https://github.com/fresh-market/fm-infra) 에 있다.
...
**요청 2만 건이 도는 동안 플랫폼 스레드가 39개에서 평평하다.** 요청이 늘어도 스레드가 안 는다 - v4 의 가상 스레드가 하는 일이 이것이다. 프로세스 CPU 는 램프 구간에 최대 70% 를 치고 그 뒤 0% 로 떨어진다. `t3.small` 2 vCPU 에 여유가 남았다는 뜻이라, 차가운 회차가 100% 로 포화됐던 것과 대비된다(7.2).
...
| 패널 | 본 것 | 무엇을 뒷받침하나 |
|---|---|---|
| HikariCP 커넥션 | active 가 **2 에서 고정** | `maximum-pool-size: 2`. **요청이 2만이어도 커넥션 수요가 안 느는 것이 이 설계의 요점이다**(5.3) |
| 톰캣 스레드 | **No data** | 가상 스레드를 켜서 톰캣 스레드 풀 자체가 안 쓰인다 |
| 힙 사용량 | 1 GiB 미만 | |
| GC 일시정지 | **최대 1ms** | 회로 지표를 켜다 GC 정지를 330ms 로 만들었던 문제가 재발하지 않았다(7.4) |

**작은 커넥션 풀과 스레드 수를 유지한 채로 처리했다.** 요청 20,000건이 도는 동안 HikariCP active 는 2, 커널 스레드는 39 에서 평평했다. **요청 수가 늘어도 커넥션과 스레드 수요가 따라 늘지 않는다**는 뜻이고, 그 상태로 p99 가 SLO 안에 들었다. 발급 요청이 트랜잭션을 안 열고 큐에 넣은 뒤 자기 때문이며, DB 를 쓰는 것은 플러시 스레드 1 과 캐시 적재 1 뿐이다(5.3).

> **다만 이것을 p99 의 공으로 돌리지 않는다.** 같은 풀과 같은 스레드 수로도 차가운 JVM 에서는 p99 가 4.69초였다(7.2). p99 를 실제로 가른 변수는 워밍업이고, 뒤에 팀이 배치 윈도와 큐 상한과 플러시 스레드를 바꿔 여섯 회차를 쟀는데 **셋 다 서로 구별되지 않았다**(7.5). **p99 를 가르는 변수는 워밍업 하나다.**

### 7.2 해석 - 값을 조여서 고칠 수 있는 문제가 아니었다
...
**이 구성에 장애를 넣어 잰 것은 [6.4](#64-장애를-실제로-넣어-재-봤다) 에 있다.**
...
5장에서 7장까지가 전부다. 요약하면 **DB 가 지킬 수 있는 것은 DB 가 지키게 하고**(R1~R4 를 스키마로), 앱은 순번 확보 하나에 집중했다. 함께 만든 것으로 Redis 재건, 정합성 검증 배치, 워밍업 러너, 이벤트 종료 정산이 있다.

### product - 두 개의 동시성 문제 (jaeungchoi / taejoong15)

**1. 상품코드를 `AUTO_INCREMENT` 에 맡길 수 없다**
...
**2. 이미지는 S3 와 DB 를 한 트랜잭션으로 묶을 수 없다**
...
주문과 결제와 장바구니도 같은 사람이 맡았다. 주문은 **가장 많이 조립하는 도메인**이라(cart 로 스냅샷, member 로 배송지, product 로 가격, stock 에 예약) 계약을 먼저 확정하고 붙였다([order-create-foundation.md](./docs/order-create-foundation.md)).
