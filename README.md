...
**이벤트가 시작되면 조건이 얼어붙는다.** 시작된 이벤트의 일정은 못 바꾸고, 소진 전이고 마감 전이면 스위치를 못 끈다. 다섯 값이 전부 고정되므로 자격 확인을 캐시할 수 있고, 요청마다 하던 쿠폰 조회가 사라진다.
...
#### 확정 표시가 실패하면 회수가 잘못 짚는다

**회수가 보는 증거는 "확정 표시가 없다" 하나인데, 그 부재를 만드는 것이 둘이다.** 정말 버려진 티켓과, 커밋은 됐는데 표시만 못 남긴 것. 스크립트는 DB 를 못 보므로 둘을 구분할 수 없다.

```
5001 이 6번으로 커밋된다 -> markCommitted 실패 -> 매핑이 "6" 인 채 pending 에 남는다
소진 + 60초 -> 회수가 5001 의 매핑을 지우고 6번을 5003 에게 준다
5003 이 INSERT -> uk_mc_coupon_seq 위반
```

**5001 의 매핑이 사라지는 것이 손해다.** 쿠폰을 갖고 있는데 다시 오면 소진 응답을 받고, `pending` 이 비었으면 410 이다.

**그 위반 자체가 감지 신호라 그 자리에서 고친다.** `SELECT member_id ... WHERE issue_seq = ?` 로 진짜 주인을 찾아 `"N:1"` 을 다시 붙이고 `pending` 에서 뺀다. 피해가 5003 한 명의 503 에서 끊기고, 그 사람은 원래 소진 응답을 받았을 사람이다.

**이 복구는 제3자를 고치는 일이라 응답 경로 위에 없다.** 그래서 배치 끝으로 미뤄 쿠폰별로 한 번에 처리한다 - 주인 조회 1회, `HDEL`/`ZREM` 각 1회, `HSET`/`ZREM` 각 1회. 티켓마다 하면 왕복이 배치 크기에 비례한다.

**실패한 표시를 다음 배치에 얹어 다시 쓰는 장치는 두지 않았다.** 위 복구만으로 상태가 수렴하고, 막는 범위가 "표시가 실패하고 그 뒤 60초 안에 소진된" 경우로 좁으며, 인스턴스 급사는 명단이 JVM 안에 있어 어차피 못 덮는다. 대신 **표시 실패를 지표로 세어** 나중에 이 판단을 재본다.
...
```
Redis timeout        100ms   왕복 셋 중 둘이 예산 밖이다 (인증 커트라인, 순번 확보)
...
**한 번은 적힌 값과 도는 값이 갈려서다.** HikariCP 의 하한 250ms 를 모르고 100 으로 적어 **적힌 값 350ms 와 도는 값 500ms 가 달랐다.** 전용 서버가 기동조차 못 해서 알았다.
...
**failover 가 있느냐, 있다면 무엇이 승격을 맡느냐로 갈린다.**

| 대상 | failover | 처리 방식 |
|---|---|---|
| ALB | 있음 (AWS 관리형) | 2 AZ 에 걸쳐 있어 한쪽 AZ 가 죽어도 남은 AZ 로 라우팅한다. 우리가 할 일이 없다 |
| 앱 (ASG) | 대기 인스턴스는 없다. **자동 교체**다 | 컨테이너 급사는 Docker restart 가 10초에 받고, 인스턴스 소실은 ASG 가 다른 AZ 서브넷에 재기동한다(4~6분). **1대로 내려가 있으면 그 시간이 그대로 다운타임이다** |
| MySQL (RDS) | **있음.** Multi-AZ 동기 복제 | standby 를 자동 승격한다. 60~120초이고 AWS 가 제어해 줄일 수 없다. 동기 복제라 **커밋 손실 0**. 앱은 그 2분을 커넥션 재연결과 재시도로 버틴다 |
| Valkey (조회 캐시) | **있음.** primary + replica 자동 페일오버 | 노드 하나면 복제본을 승격한다. 복제가 비동기라 지연분은 잃는다. **전량 소실이면 failover 가 아니라 강등**이다. 원본 조회로 떨어지고 적재 실패는 무시한다(30초) |
| **Valkey (이벤트 중 순번)** | 있음. 다만 **승격만으로 안 끝난다** | 순번의 판정 주체라 승격 뒤에도 키가 비어 있을 수 있다. 그때는 **DB 의 `MAX(issue_seq)` 와 앱 큐로 네 키를 재건**하고, 그동안 스크립트가 `-2` 로 요청을 막는다(아래) |
| 배치 | **없음.** 단독 인스턴스 | 대체를 두지 않는다. **프로세스가 항상 하나인 것이 전제**라 이중화하면 같은 배치가 겹쳐 돈다. 다음 주기에 따라잡고, 상태 전이가 조건부 UPDATE 라 겹쳐 돌아도 한 번만 반영된다 |
| 모니터링 | **없음.** 단독 인스턴스 | 죽으면 감지 수단이 사라진다. 핵심 알람만 CloudWatch 에 이중으로 걸어 두 번째 경로를 만든다(30분) |

**readiness 에 DB 를 넣지 않는다.** 넣으면 DB failover 2분 동안 모든 인스턴스가 동시에 대상 그룹에서 빠져 상황이 더 나빠진다. 공유 의존성은 헬스체크에서 제외하고 인스턴스 로컬 상태만 넣는다.
...
**작은 커넥션 풀과 스레드 수를 유지한 채로 처리했다.** 요청 20,000건이 도는 동안 HikariCP active 는 2, 커널 스레드는 39 에서 평평했다. **요청 수가 늘어도 커넥션과 스레드 수요가 따라 늘지 않는다**는 뜻이고, 그 상태로 p99 가 SLO 안에 들었다. 발급 요청이 트랜잭션을 안 열고 큐에 넣은 뒤 자기 때문이며, DB 를 쓰는 것은 플러시 스레드 1 과 캐시 적재 1 뿐이다(5.7).

> **다만 이것을 p99 의 공으로 돌리지 않는다.** 같은 풀과 같은 스레드 수로도 차가운 JVM 에서는 p99 가 4.69초였다(6.3). p99 를 실제로 가른 변수는 워밍업이고, **풀 크기를 바꿔 가며 잰 회차가 없어** "작은 풀이 빠르다" 고는 말할 수 없다. 지금 말할 수 있는 것은 "작으면서도 SLO 를 지켰다" 까지다.
