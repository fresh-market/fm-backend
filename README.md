...
|---|---|---|---|
| **앱 인스턴스 급사** | **응답도 못 간 채 큐째로 사라진다** | 상태를 안 들고 있어 ASG 교체가 곧 복구다 | 사용자 재시도시 발급 / 60초 뒤 회수 |
| **Redis 사망**<br/>primary + replica | **전부 완료된다** | 대체 발급기 없이 새 요청만 혼잡(503)으로 끊는다 | 복구 뒤 재건이 `free` 를 채운다 |
| **RDB 사망**<br/>primary + standby | **전부 실패한다** | 회로를 열어 순번 확보 자체를 건너뛴다 | 사용자 재시도시 발급 / 60초 뒤 회수 |
