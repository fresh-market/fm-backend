...
**대신 이미 도는 경로가 장애도 받게 만든다.** 재시도와 조건부 UPDATE 와 유니크 제약처럼 평상시에 늘 실행되는 것들이다.

**그리고 readiness 에 공유 의존성을 안 넣는다.** DB 를 넣으면 페일오버가 도는 동안 **모든 인스턴스가 동시에 대상 그룹에서 빠진다.** 한 대의 문제가 아니라 전체가 사라지는 것이라 상황이 더 나빠진다. **공유 의존성이 죽은 것은 인스턴스를 빼서 고칠 수 있는 문제가 아니다.**
...
**일곱 중 넷은 앱이 할 일이 아예 없다.** 죽어도 발급과 주문이 안 멈춘다는 뜻이다.

**배치를 이중화하지 않는 것은 비용 때문이 아니다.** 프로세스가 항상 하나인 것이 전제라 겹쳐 돌면 같은 배치가 두 번 돈다. 그래서 이중화 대신 **상태 전이를 조건부 UPDATE 로 두어 겹쳐 돌아도 한 번만 반영되게** 만들었다.

### 6.3 회로가 재고를 지키지 않고 이벤트를 망가뜨렸다

**부하 시험에서 찾은 것 중 가장 큰 소득이다.** 같은 코드, 같은 차가운 JVM, 순번 확보 회로만 껐다.
...
**밝히는 데 하루가 걸린 이유가 따로 있다.** 팀이 회로를 레지스트리 밖에서 만들어 **Micrometer 가 그것을 못 봤다.** 회로가 열려도 지표에 흔적이 없었다. 게다가 등록해서 고치려다 **지표를 보려던 행위가 지표가 재려던 현상을 만들었다.** 이벤트 소비자가 붙자 호출마다 객체가 생겨 GC 정지가 세 배가 됐다. 지금은 설정을 스타터로 옮겨 이 문제가 없다.

> 회로를 `couponSeq` 와 `couponWrite` 둘로 나눈 이유, 플러시를 안 막는 이유는 [coupon-failure.md](./docs/coupon/coupon-failure.md) 1장에 있다.

### 6.4 장애를 실제로 넣어 재 봤다

**팀은 AWS 에 인프라를 올려 가장 좋은 구성에 장애를 넣었다.** 앱 급사(SIGKILL), 캐시 Multi-AZ 페일오버, 둘을 겹친 것 셋이다. **DB 페일오버는 뺐다.** DB 가 죽으면 애초에 발급이 안 되므로 잴 것이 없다.
