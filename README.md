...
| | 진행 중이던 발급 | 앱이 하는 일 | 묶인 번호를 푸는 쪽 |
|---|---|---|---|
| **앱 인스턴스 급사** | **큐째로 사라진다.** 응답도 못 간다 | 없다. 상태를 안 들어서 ASG 교체가 곧 복구다 | 사용자 재시도. 안 오면 회수 |
| **Redis 사망** | **전부 완료된다** | 새 요청을 혼잡으로 끊는다. 대체 발급기를 안 만든다 | 복구 뒤 재건이 `free` 를 채운다 |
...
**2. Redis 가 죽으면 진행 중인 발급은 안 끊긴다.** 큐 항목이 번호와 회원과 응답 통로뿐이고 플러시 스레드는 DB 에만 쓰기 때문이다. **셋 중 진행 중인 발급에 가장 무해하다.** 앱은 새 요청만 혼잡으로 끊는다. **대체 순번 발급기를 두지 않는다.**
