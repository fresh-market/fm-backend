...
| 키 | 무엇 |
|---|---|
| `coupon:{id}:seq` | 회원 -> 순번 해시. 값이 `"6"` 이면 미확정, `"6:1"` 이면 커밋까지 끝났다 |
| `coupon:{id}:free` | 실패해 반납된 번호. **`ZPOPMIN` 이 `INCR` 보다 먼저 돈다** |
| `coupon:{id}:counter` | **마지막으로 나간 번호.** `INCR` 결과가 곧 발급 번호다 |
| `coupon:{id}:pending` | 번호를 받고 아직 행이 안 된 것. 회수 대상. 점수가 발급 시각이다 |

넷은 함께 살고 함께 죽는다. 수명은 `counter` 가 들고 있고(`issue_end_at + 60초`) 나머지 셋은 만들어지는 자리에서 그 만료 시각을 물려받는다.
...
**반납이 `DECR` 이 아닌 이유.** A 가 6번, B 가 7번을 받고 A 가 실패하면 `DECR` 은 **맨 위 번호(7)** 를 회수하므로 다음 사람이 B 의 번호를 받는다. 그래서 번호를 **지목해** `free` 정렬 집합에 넣는다. 리스트가 아닌 이유는 같은 번호가 두 번 들어가면 두 사람이 같은 번호를 받기 때문이다.

**앱이 급사하면 반납 주체가 사라진다.** 그때는 소진 시점에 스크립트가 `pending` 에서 기준 시간(`reclaim-after` 60초)을 넘긴 번호를 회수한다. 주기 배치가 필요 없고, 재고가 남은 동안에는 그 줄에 닿지도 않는다.
