...
flowchart TB
    ...
    subgraph COUPON["선착순 전용 ASG 의 앱 인스턴스 (t3.small, min 0 / max 3, 대수만큼 같은 구조)"]
        ...
        Q1 --> F1["플러시 스레드 1 (플랫폼)<br/>쓰는 커넥션 1, 풀 최대 2"]
    end
    TGC --> VT
...
**정상 구간에 인스턴스 하나가 실제로 쓰는 커넥션은 1개다.** 요청 스레드는 트랜잭션을 안 열고, `flush-threads` 가 1이라 DB 에 계속 쓰는 것이 그 스레드 하나다. **풀을 2로 잡은 것은 `minimum-idle` 이 1이기 때문**이고, 플러시가 하나를 빌려 가면 Hikari 가 유휴 하한을 채우려고 하나를 더 세워 둔다. 둘째는 서 있는 것이지 쓰이는 것이 아니다.

**이벤트가 시작될 때만 잠깐 둘이 된다.** 쿠폰 스냅샷 캐시가 그때 한 번 DB 를 채우기 때문이다. 조건이 고정되므로 그 뒤로는 캐시가 DB 를 안 친다.
...
| 패널 | 본 것 | 무엇을 뒷받침하나 |
|---|---|---|
| HikariCP 커넥션 | 풀이 **2 에서 고정** | `maximum-pool-size: 2`, 그중 쓰는 것은 1. **요청이 2만이어도 커넥션 수요가 안 느는 것이 이 설계의 요점이다**(5.0) |
