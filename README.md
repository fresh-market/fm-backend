...
**어느 버전도 쓰기를 미루지 않는다.** 표의 큐와 묶음 쓰기는 **묶어서 쓰되 커밋까지 기다리는 것**이라, 앱은 행이 들어간 뒤에야 발급됐다고 답한다. 정합성이 그 조건에 걸려 있어 버전을 올리면서도 이것만은 안 바꿨다(5.4).
...
**복구는 페일오버보다 오래 걸린다.** 캐시 페일오버 자체는 20초인데 **순번 확보가 막힌 구간은 45초를 넘었다.** 캐시가 돌아와도 회로가 반열림을 거쳐야 닫히고, **닫히는 순간 밀린 요청과 재시도가 한꺼번에 들어오기 때문이다.** 실패가 장애 중이 아니라 그 시점에 몰린다.
