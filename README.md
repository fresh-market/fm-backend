...
| [6. 부하 시험: 결과, 해석, 개선](#6-부하-시험-결과-해석-개선) | 통과보다 오진을 찾은 것이 소득이다. 그리고 실제 인프라에서 다시 쟀다 |
...
### 5.7 죽었을 때 대체 경로를 새로 만들지 않는다

**이것이 장애 대응 전체를 관통하는 원칙이다.** 대체 경로는 평소에 안 돌아 **정작 필요한 날 처음 실행된다.**

그래서 팀은 세 가지를 안 만들었다.

```
대체 순번 발급기      Redis 가 죽으면 순번을 못 받은 요청을 혼잡으로 끊는다.
                     이미 순번을 받아 큐에 있는 요청은 그대로 발급된다
재건용 차단 장치      스크립트가 counter 없이 -2 를 주므로,
                     재건이 카운터를 마지막에 쓰는 것이 곧 문을 여는 동작이다
배치 이중화          프로세스가 항상 하나인 것이 전제다.  겹쳐 돌면 같은 배치가 두 번 돈다
```

**그리고 readiness 에 DB 를 넣지 않는다.** 넣으면 DB 페일오버 2분 동안 **모든 인스턴스가 동시에 대상 그룹에서 빠져** 상황이 더 나빠진다. 공유 의존성은 헬스체크에서 빼고 인스턴스 로컬 상태만 넣는다.

> 구성 요소별 페일오버 여부와 그때 앱이 하는 일은 [coupon.md 8장](./docs/coupon/coupon.md), 재건 절차는 [redis-promotion-rebuild.md](./docs/coupon/redis-promotion-rebuild.md), 회수 기준 시간의 근거는 [coupon-reclaim-after.md](./docs/coupon/coupon-reclaim-after.md) 에 있다.
...
**그 뒤에 팀은 AWS 에 인프라를 올려 회차 19개와 장애 회차 3개를 더 돌렸다**(6.6). 회차를 왜 그렇게 짰는지는 [coupon-load-scenarios.md](./docs/coupon/coupon-load-scenarios.md), 회차별 값은 [coupon-load-rounds.md](./docs/coupon/coupon-load-rounds.md) 에 있다. 초기 회차 기록은 [`fm-infra`](https://github.com/fresh-market/fm-infra) 에 있다.
...
> **다만 이것을 p99 의 공으로 돌리지 않는다.** 같은 풀과 같은 스레드 수로도 차가운 JVM 에서는 p99 가 4.69초였다(6.3). p99 를 실제로 가른 변수는 워밍업이고, 뒤에 팀이 배치 윈도와 큐 상한과 플러시 스레드를 바꿔 여섯 회차를 쟀는데 **셋 다 서로 구별되지 않았다**(6.6). **p99 를 가르는 변수는 워밍업 하나다.**
...
**그리고 가장 좋은 구성에 장애를 넣었다.** 앱 급사(SIGKILL), 캐시 Multi-AZ 페일오버, 둘을 겹친 것 셋이다. DB 페일오버는 뺐다. **DB 가 죽으면 애초에 발급이 안 되므로 잴 것이 없다.**
...
**약한 쪽 근거만 적혀 있어서 팀이 10초로 내렸다가 망가뜨렸다.** 회수 가드가 읽는 확정 표시는 `markCommitted` 가 붙이는데 **그것도 Redis 호출이라 Redis 가 죽으면 가드가 무력해진다.** 회수가 살아 있는 번호를 가져가 유니크 제약에 걸린다.

**실패는 장애 중이 아니라 회로가 닫히는 순간에 몰린다.** 캐시 페일오버 실측이 20초대인데 순번 확보가 막힌 구간은 45초를 넘었다. 회로가 반열림을 거쳐 닫히고, **닫히는 순간 밀린 요청과 재시도가 한꺼번에 들어와 같은 회원의 요청이 겹치기 때문이다.**

> 회차별 값은 [coupon-load-rounds.md](./docs/coupon/coupon-load-rounds.md), 기준 시간을 고른 근거는 [coupon-reclaim-after.md](./docs/coupon/coupon-reclaim-after.md) 에 있다.
