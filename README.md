...
**1. 이 그룹은 쿠폰 부하만 받는다. 이것이 가장 크다.** 평상시 앱과 인스턴스가 겹치지 않으므로 **다른 기능의 성능에 영향이 없고**, 잰 수치가 오직 발급 경로의 것이 된다. 그래서 팀은 운영에 올리기 전에 개발 서버에서 **같은 모양의 예측 가능한 환경**으로 미리 대비할 수 있다. 대수와 프로필과 경로가 같으면 개발에서 본 것이 운영에서도 그대로 나온다 - 7장의 회차별 비교가 성립하는 것도 이 조건 덕분이다.
...
**인스턴스 하나가 쓰는 커넥션은 둘뿐이고, 그중 계속 쓰는 것은 하나다.**

```
플러시 스레드   1   DB 에 쓰는 것은 이것뿐이다 (flush-threads: 1)
캐시 적재      1   이벤트가 열릴 때 한 번 채운다
```

**요청 스레드는 트랜잭션을 안 연다.** 그래서 요청이 2만이어도 이 둘 말고 커넥션을 더 쥐는 것이 없다.

**캐시는 이벤트 내내 DB 를 다시 안 친다.** 발급 창 안에서 다섯 값이 고정되므로 캐시 항목이 마감까지 살아 있다. 그래서 **이벤트가 도는 동안 계속 커넥션을 쓰는 것은 플러시 스레드 하나다.**

**그래도 풀이 2인 것은 그 둘을 갈라 놓으려는 것이다.** 1로 줄이면 캐시를 채우는 동안 플러시가 기다리고, 그 대기가 `slow-call-duration` 을 넘기면 **DB 쓰기 회로가 열려 그 배치가 통째로 밀린다.**
...
**둘의 차이가 수십 배다.** 약한 쪽만 적혀 있어서 팀이 10초로 내렸다가 발급이 깨졌다. 회수 가드가 읽는 확정 표시는 `markCommitted` 가 붙이는데 **그것도 Redis 호출이라 Redis 가 죽으면 표시가 안 붙는다.** 회수가 **이미 DB 에 있는 번호를 남에게 넘겨** 유니크 제약에 걸린다.
...
| 패널 | 본 것 | 무엇을 뒷받침하나 |
|---|---|---|
| HikariCP 커넥션 | 풀이 **2 에서 고정** | `maximum-pool-size: 2`. **요청이 2만이어도 커넥션 수요가 안 느는 것이 이 설계의 요점이다**(5.0) |
| 톰캣 스레드 | **No data** | 가상 스레드를 켜서 톰캣 스레드 풀 자체가 안 쓰인다 |
| 힙 사용량 | 1 GiB 미만 | |
| GC 일시정지 | **최대 1ms** | 회로 지표를 켜다 GC 정지를 330ms 로 만들었던 문제가 재발하지 않았다(7.4) |

**작은 커넥션 풀과 스레드 수를 유지한 채로 처리했다.** 요청 20,000건이 도는 동안 HikariCP 풀은 2, 커널 스레드는 39 에서 평평했다. **요청 수가 늘어도 커넥션과 스레드 수요가 따라 늘지 않는다**는 뜻이고, 그 상태로 p99 가 SLO 안에 들었다. 발급 요청이 트랜잭션을 안 열고 큐에 넣은 뒤 자기 때문이며, DB 를 쓰는 것은 플러시 스레드 1 과 캐시 적재 1 뿐이다(5.3).
...
**이 구성에 장애를 주입해 계측한 것은 [6.4](#64-장애-주입과-계측) 에 있다.**
