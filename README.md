...
### 6.4 장애를 실제로 주입해 계측했다

**팀은 AWS 에 인프라를 올려 가장 좋은 구성에 장애를 주입했다.** 앱 급사(SIGKILL), 캐시 Multi-AZ 페일오버, 둘을 겹친 것 셋이다. **팀은 DB 페일오버를 뺐다.** DB 가 죽으면 애초에 발급이 안 되므로 잴 것이 없다.
...
**이 구성에 장애를 주입해 계측한 것은 [6.4](#64-장애를-실제로-주입해-계측했다) 에 있다.**
