...
**대신 회로가 번호를 덜 태운다.** DB 쓰기 실패가 쌓여 회로가 `OPEN` 이 되면 요청 스레드가 `INCR` 자체를 건너뛰어 **묶일 번호가 애초에 안 생긴다.**
...
**복구는 페일오버보다 오래 걸린다.** 캐시 페일오버 자체는 20초인데 **순번 확보가 막힌 구간은 45초를 넘었다.** 캐시가 돌아와도 회로가 `HALF_OPEN` 을 거쳐야 `CLOSED` 로 돌아가고, **닫히는 순간 밀린 요청과 재시도가 한꺼번에 들어오기 때문이다.** 실패가 장애 중이 아니라 그 시점에 몰린다.
...
* **회로가 `CLOSED` 로 돌아가고 밀린 것이 다 빠지기까지 걸린 시간.** 이 값이 회수 기준 시간의 진짜 하한인데 팀에게 그 지표가 없다
